...
11. **WB + Sandbag FREQUENCY (Block 3) — ~3 touches/wk each, but only ~1 HARD (decided 2026-08-16).** Frequency helps skill/durability (his need), BUT recovery is the #1 limiter and BOTH joints are gated (knee=WB, adductor=sandbag) → **NEVER 3 hard sessions each.** Tier it:
    - **Wall balls (knee-gated):** 1 HARD compromised/coupled (wrecked, after the engine = the durability driver) + 1 moderate EMOM (fresh pacing) + 1 OPTIONAL light skill dose = up to 3 touches, only 1 taxing.
