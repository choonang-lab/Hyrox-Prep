...
9. **WORKED EXAMPLE — the Block 3 compromised session (adapted from a coach template, 2026-08-09).** This is the template for the compromised/coupled slot (item 1). Use **Part 2 as the core session; Part 1 optional.**
   ...
   - **SANDBAG durability session (added 2026-08-16, from an aj_hyrox template):** primary format = **5×90s repeatable-distance lunges (90s work / 30s rest, "find a distance you can hold across all 5")** — best specificity to his gap (sustained continuous distance under accumulating fatigue; he currently breaks 100m into chunks). EMOM (shorter bouts, more rest) = the **adductor-safe ON-RAMP** to build tolerance first, then progress to the 5×90s. **5×90s is the highest eccentric-adductor load of any lunge format → introduce conservatively (start 3-4 rounds, scaled distance/load ~20-22.5kg, adductor-gated, stop on groin talk, NEVER the day before a running day).** Use % of a tested/known repeatable distance, small progressions (per the template). SKIP the template's "build each leg" strength block (step-ups = knee-flare/cut; his lunge limiter is distance+durability, NOT max leg strength) and its "train late/fatigued" point is already the compromised-work thesis.
