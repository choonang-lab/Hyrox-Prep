...
9. **WORKED EXAMPLE — the Block 3 compromised session (adapted from a coach template, 2026-08-09).** This is the template for the compromised/coupled slot (item 1). Use **Part 2 as the core session; Part 1 optional.**
   ...
   - **SANDBAG = his declared FOCUS: 2 HARD sessions/wk (confirmed 2026-08-17), varied stimulus, + light 15kg pattern doses for frequency.** He wanted 2-3 hard; capped at **2, NOT 3** (adductor is the ceiling — 3 hard lunge sessions/wk during a ramp is where groin strains happen; asymmetric downside). **Fund it by CUTTING Bulgarian split squats** (unilateral leg work competing for the same adductor/knee budget) + accessories; do NOT cut WB (bigger gap), the running floor, or the sim. The 3rd hard touch is EARNED only after ~3-4 wk of 2-hard with a dead-quiet adductor.
     ...
     - **HARD #2 — DURABILITY (compromised, run-coupled) [designed 2026-08-17]:** warm-up + adductor prep → **4 rounds of [400m run @ race/threshold → 20m sandbag @20kg UNBROKEN → <45s transition]** → **FINISH 1×1000m @ race pace off the last sandbag** (the run-off-sandbag fade = "run 8 lives or dies" = his exact leak). ENTRY conservative: 3 rounds × [400m + 15m] + 800m finish; build ONE variable at a time (chunk 15→20→25m OR +round) as the adductor allows; endgame = 100m broken into 2-3 larger chunks. METRICS: chunk UNBROKEN each round (pacing signal — break = too big), finishing-km fade vs fresh km (the durability adaptation), adductor quiet during + next-day.
